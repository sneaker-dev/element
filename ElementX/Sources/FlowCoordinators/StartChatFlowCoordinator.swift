...
class StartChatFlowCoordinator: FlowCoordinatorProtocol {
    ...
    enum Event: EventType {
        ...
        /// The user would like to open the dial pad.
        case openDialPad
        /// The user dismissed the dial pad screen.
        case dismissedDialPad
        /// The user created a room from the dial pad.
        case createdRoomFromDialPad
    }
    ...
    private func configureStateMachine() {
        ...
        stateMachine.addRoutes(event: .openDialPad, transitions: [.startChat => .dialPad]) { [weak self] _ in
        ...
        stateMachine.addRoutes(event: .createdRoomFromDialPad, transitions: [.dialPad => .startChat]) { [weak self] context in
            guard let roomProxy = context.userInfo as? JoinedRoomProxyProtocol else {
                fatalError("A room proxy is required.")
            }
            // Navigate directly to the room, skipping invite users
            self?.actionsSubject.send(.finished(roomID: roomProxy.id))
        }
    ...
    private func presentStartChatScreen() {
        ...
        coordinator.actions.sink { [weak self] action in
            ...
            case .openDialPad:
                stateMachine.tryEvent(.openDialPad)
            }
        }
    ...
    private func presentDialPadScreen() {
        ...
        coordinator.actions.sink { [weak self] action in
            ...
            switch action {
            case .createdRoom(let roomProxy):
                stateMachine.tryEvent(.createdRoomFromDialPad, userInfo: roomProxy)
            }
        }
        .store(in: &cancellables)
